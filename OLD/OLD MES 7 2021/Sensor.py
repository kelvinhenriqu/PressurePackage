...
def SaveSD(P,T,Filename):
    ...
    timestamp = datetime.now().strftime('%I%M%S%f')
    ...
    with open(Directory, mode='a') as sensor_readings:
        ...
        write_to_log = sensor_write.writerow([P,T,timestamp]) #write Pressure & Temperature and Timestamp in sdcard
        return(write_to_log)
...
class Measurement :
    ...
    def GetValue(measure,filename):
        ...
        try:        
            ...
            Pressure = round(ipress, 3)  #round the number to 3 number after dot
            ...
            Temperature = round(itemp,1) #round the number to 3 number after dot

            Save = SaveSD(Pressure,Temperature,filename) #save Pressure and Temperature in filename 
